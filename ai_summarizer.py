...
def _format_lab_summary_for_prompt(lab_summary: Dict[str, Any]) -> str:
    ...
    for lab_key, lab_data in lab_summary.items():
        ...
        if value is not None:
            ...
            range_str = f" (Normal: {min_val}-{max_val})" if min_val is not None and max_val is not None else ""
            status = ""
            ...
            lines.append(f"- {name}: {value}{range_str}{status} (Captured: {captured_ago})")
...
def _call_huggingface_api(
    ...
    if not model_name:
        model_name = "microsoft/DialoGPT-medium"  # Fallback, but we'll try better ones first
...
def _generate_mock_summary(risk_assessment: Dict[str, Any]) -> str:
    ...
    ariscat_percentage = pulmonary_risks.get("ARISCAT", {}).get("score_percentage", "N/A")
    
    recommendations = risk_assessment.get("recommendations", [])
    top_recommendation = recommendations[0] if recommendations else "Standard pre-operative monitoring recommended."
...
if __name__ == "__main__":
    ...
    print("Attempting to generate summary using local AI model (free, no internet required)...")
    ...
    print("   result = summarize_risk_assessment(risk_assessment, api_provider='local')")
    print("   Note: First run downloads the model (~1.6GB), then works offline")
    print("\n2. Hugging Face API (Free, requires internet):")
    print("   result = summarize_risk_assessment(risk_assessment, api_provider='huggingface')")
    ...
    print("   result = summarize_risk_assessment(risk_assessment, api_provider='gemini')")
