...
class PatientClinicalData:
    ...
    devices: List[str] = field(default_factory=list)  # ['CPAP', 'BIPAP', 'pacemaker', 'ICD', 'insulin_pump', 'CGM']
    ...
    recent_cardiac_event_months: Optional[int] = None  # Months since last MI/PCI/CHF admission
    ...
    diabetes_complications: List[str] = field(default_factory=list)  # e.g., ['retinopathy', 'nephropathy']
...
class MedicationTrigger:
    ...
    name_patterns: List[str] = field(default_factory=list)  # Specific medication name patterns
    min_dose: Optional[float] = None  # Minimum dose threshold (e.g., 40mg furosemide equivalent)
...
class TriggerConditions:
    ...
    icd10Codes: List[str] = field(default_factory=list)  # Exact codes or prefixes (e.g., "I25.10", "E11.*")
    medicationTriggers: List[MedicationTrigger] = field(default_factory=list)  # Enhanced medication matching
    medicationClasses: List[str] = field(default_factory=list)  # Simple class matching (backward compat)
    labThresholds: List[LabThreshold] = field(default_factory=list)
    deviceTypes: List[str] = field(default_factory=list)
    requiredFlags: List[str] = field(default_factory=list)  # e.g., ['chestPain', 'shortnessOfBreath']
    demographicConditions: List[str] = field(default_factory=list)  # e.g., ['age >= 65']
    secondaryTriggers: Optional[TriggerConditions] = None  # Secondary triggers for review flags
...
class RiskStratificationRules:
    ...
    intermediateRisk: List[str] = field(default_factory=list)  # Conditions for intermediate risk
...
class ComorbidityBlockDefinition:
    ...
    riskStratification: RiskStratificationRules = field(default_factory=RiskStratificationRules)
...
class ConfidenceCalculator:
    ...
    @staticmethod
    def get_confidence_tier(score: float) -> Literal['very_high', 'high', 'moderate', 'low', 'very_low']:
        """
    ...
    def calculate_temporal_confidence(
        ...
        recency_component = sum(recency_scores) / len(recency_scores) if recency_scores else 0.5
        
        # Consistency over time component (weighted 0.4)
        consistency_base = sum(consistency_scores) / len(consistency_scores) if consistency_scores else 0.5
    ...
    def calculate_consistency_confidence(
        ...
        confirming_sources = len([s for s in sources if s in ['problem_list', 'medications', 'labs']])
...
class TriggerEvaluationResult:
    ...
    confidenceTier: Literal['very_high', 'high', 'moderate', 'low', 'very_low'] = 'very_low'
    confidenceFlag: Optional[str] = None  # 'low_confidence_require_review', 'standard_confidence_suggest_review', 'confident_display_optional_review', 'high_confidence_minimal_review', None
    riskLevel: Literal['low', 'intermediate', 'high'] = 'low'
    requiredActions: List[str] = field(default_factory=list)  # Deprecated: use categorizedActions
    categorizedActions: CategorizedActions = field(default_factory=lambda: CategorizedActions())
    ...
    exportNote: Optional[str] = None  # Note to include in export for low confidence blocks
...
def calculate_egfr_ckd_epi(
    ...
    # Calculate eGFR
    egfr = 141 * (min(creatinine / kappa, 1.0) ** alpha) * (max(creatinine / kappa, 1.0) ** -1.209) * (0.993 ** age) * race_multiplier
    
    return max(egfr, 0.0)  # Ensure non-negative


def match_icd10_code(
    patient_code: str,
...
def evaluate_trigger(
    ...
    primary_trigger_confidences: List[float] = []  # Track individual trigger confidences
    ...
    for problem_item in patient_data.problemList:
        ...
        if problem_date_str:
            try:
                problem_date = datetime.fromisoformat(problem_date_str.replace('Z', '+00:00'))
                years_old = (datetime.now() - problem_date.replace(tzinfo=None)).days / 365.25
    ...
    for trigger in block.triggers.medicationTriggers:
        for med in patient_data.medications:
            if match_medication_trigger(med, trigger):
                ...
                specific_classes = ['sglt2_inhibitor', 'arni', 'loop_diuretic_high_dose']
    ...
    for condition in block.triggers.demographicConditions:
        if condition.startswith('age >='):
            try:
                ...
                if patient_data.demographics.age is not None and patient_data.demographics.age >= threshold_age:
                    triggered = True
                    primary_triggers += 1
                    # Demographic conditions = 0.8 (reliable but less specific)
                    primary_trigger_confidences.append(0.8)
                    trigger_reasons.append(f"Demographic condition met: age >= {threshold_age}")
            except:
                pass
    ...
    if block.triggers.secondaryTriggers:
        ...
        if vitals.heartRate is not None:
            if vitals.heartRate > 100 or vitals.heartRate < 50:
                secondary_triggers += 1
                trigger_reasons.append(f"[Secondary] Abnormal heart rate: {vitals.heartRate}")
    ...
    if triggered:
        ...
        if block.blockId == 'DIABETES_001':
            ...
            insulin_classes = ['insulin', 'rapid_acting_insulin', 'long_acting_insulin', 'intermediate_acting_insulin']
            has_insulin = any(med.class_.lower() in insulin_classes for med in patient_data.medications)
            if has_insulin:
                diabetes_primary_confidences.append(0.8)  # Insulin use: 0.8
            
            # Check for other DM meds (non-insulin diabetes medications)
            dm_med_classes = ['metformin', 'sglt2_inhibitor', 'glp1_agonist', 'dpp4_inhibitor', 
                             'sulfonylurea', 'thiazolidinedione', 'meglitinide', 'alpha_glucosidase_inhibitor']
            has_other_dm_meds = any(med.class_.lower() in dm_med_classes for med in patient_data.medications)
            if has_other_dm_meds and not has_insulin:  # Only count if not already counted as insulin
        ...
        # OSA-specific primary triggers
            ...
            has_cpap_prescription = any(med.class_.lower() in cpap_med_classes for med in patient_data.medications)
        ...
        # OSA-specific secondary triggers
            ...
            # BMI > 35
            if patient_data.demographics.bmi is not None and patient_data.demographics.bmi > 35:
                stop_bang_score += 1
            
            # Age > 50
            if patient_data.demographics.age is not None and patient_data.demographics.age > 50:
                stop_bang_score += 1
            ...
            # Pressure (BP) - high BP
            if patient_data.vitals.bloodPressure and patient_data.vitals.bloodPressure.systolic is not None:
        ...
        vitals = patient_data.vitals
        if vitals.heartRate is not None and (vitals.heartRate > 100 or vitals.heartRate < 50):
        ...
        # OSA-specific temporal factors
            ...
            for problem_item in patient_data.problemList:
                if problem_item.date:
                    ...
                    if problem_item.code == 'G47.33' or 'sleep' in problem_item.name.lower():
                        if recency_score >= 0.8:  # Within 90 days
        ...
        has_medications = any(match_medication_trigger(med, trigger) 
                             for trigger in block.triggers.medicationTriggers 
                             for med in patient_data.medications) or \
                         any(mc.lower() in patient_med_classes 
                             for mc in block.triggers.medicationClasses)
        has_labs = any(evaluate_lab_threshold(lab_map.get(threshold.test), threshold) 
                      for threshold in block.triggers.labThresholds)
        ...
        if block.blockId == 'CAD_CHF_001':
            ...
            if has_problem_list:
                ...
                if patient_data.labs.troponin is not None and patient_data.labs.troponin <= 0.04:
                ...
                if patient_data.labs.bnp is None and 'heart_failure' in [code.lower() for code in matching_codes]:
            ...
            if has_problem_list:
                if patient_data.labs.bilirubin is not None and patient_data.labs.bilirubin < 2.0:
    ...
    if triggered:
        ...
        if block.riskStratification.highRisk or block.riskStratification.intermediateRisk:
    ...
    if triggered:
        ...
        if secondary_triggers > 0:
            base_actions.append("Review secondary trigger flags for additional considerations.")
        ...
        if confidence >= 0.7:
            ...
            for action in base_actions:
                ...
                if 'ekg' in action_lower or 'ecg' in action_lower:
                    ...
                    if 'cardiology' in action_lower:
                    ...
                    elif 'specialist' in action_lower or 'nephrology' in action_lower or 'endocrinology' in action_lower:
                        ...
                        # Generic consult
                        required_action = action.replace('Consider', 'Consult').replace('consider', 'Consult')
                    ...
                    if action_lower.startswith(('consider', 'may', 'could')):
                        required_action = action.replace('Consider', 'Obtain').replace('consider', 'Obtain')
                        required_action = required_action.replace('May benefit from', 'Check').replace('may benefit from', 'Check')
                        required_action = required_action.replace('Could be', '').replace('could be', '')
            ...
            for action in base_actions:
                ...
                if 'ekg' in action_lower or 'ecg' in action_lower:
                    ...
                    if 'cardiology' in action_lower:
                        ...
                        suggested_action = action.replace('Consult', 'Consult could be considered').replace('consult', 'consult could be considered')
                else:
                ...
                if 'ekg' in action_lower or 'ecg' in action_lower:
                    ...
                    if 'cardiology' in action_lower:
                        ...
                        if 'nephrology' in action_lower:
                            ...
                            specialist_type = "endocrinology"
                        optional_action = f"{specialist_type.capitalize()} input may be helpful"
                    else:
                        optional_action = action.replace('Consult', 'Consult input may be helpful').replace('consult', 'consult input may be helpful')
                else:
                    # Convert general actions to optional format
                    if 'could' not in action_lower and 'may' not in action_lower:
                        optional_action = f"{action} could be obtained if clinical concern"
        ...
        if confidence_flag == 'low_confidence_require_review':
            ...
            export_allowed = True
            export_warning = "Low confidence comorbidity block - clinical correlation recommended"
            export_note = "Low confidence comorbidity identified - clinical correlation recommended"
            # Require clinician review
            if optional_actions:
                optional_actions.insert(0, "⚠️ LOW CONFIDENCE: Clinician review REQUIRED - evidence is weak or incomplete. Verify diagnosis before proceeding.")
            else:
                optional_actions.append("⚠️ LOW CONFIDENCE: Clinician review REQUIRED - evidence is weak or incomplete. Verify diagnosis before proceeding.")
            ...
            if suggested_actions:
                suggested_actions.insert(0, "ℹ️ STANDARD CONFIDENCE: Clinician review suggested - verify diagnosis appropriateness.")
            else:
                suggested_actions.append("ℹ️ STANDARD CONFIDENCE: Clinician review suggested - verify diagnosis appropriateness.")
...
def assess_risk_level(
    ...
    if block.blockId == 'CAD_CHF_001':
        ...
        if (patient_data.lvef is not None and patient_data.lvef < 40) or \
           (patient_data.labs.bnp is not None and patient_data.labs.bnp > 400) or \
           (patient_data.recent_cardiac_event_months is not None and patient_data.recent_cardiac_event_months < 6) or \
           (patient_data.inotropic_dependence) or \
           (patient_data.nyha_class in ['III', 'IV']):
            return 'high'
        
        # INTERMEDIATE RISK: Stable symptoms, LVEF 40-50%, BNP 100-400
        if (patient_data.lvef is not None and 40 <= patient_data.lvef <= 50) or \
           (patient_data.labs.bnp is not None and 100 <= patient_data.labs.bnp <= 400) or \
           (patient_data.nyha_class == 'II') or \
        ...
        if (patient_data.labs.a1c is not None and patient_data.labs.a1c > 9.0) or \
           (len(patient_data.diabetes_complications) > 0) or \
           (patient_data.recurrent_hypoglycemia):
            return 'high'
        ...
        eGFR = None
        if patient_data.labs.creatinine is not None and patient_data.demographics.age is not None:
            eGFR = calculate_egfr_ckd_epi(
                creatinine=patient_data.labs.creatinine,
                age=patient_data.demographics.age,
                sex=patient_data.demographics.sex or 'M'
        ...
        if (eGFR is not None and eGFR < 30) or \
           (patient_data.dialysis_dependent) or \
           (patient_data.labs.sodium is not None and (patient_data.labs.sodium < 135 or patient_data.labs.sodium > 145)):
            return 'high'
        
        # INTERMEDIATE RISK: eGFR 30-44, proteinuria present
        if (eGFR is not None and 30 <= eGFR < 45) or \
           (patient_data.proteinuria is True):
            return 'intermediate' if initial_risk != 'high' else 'high'
...
def get_risk_specific_recommendations(
    ...
    if risk_level == 'high':
        recommendations.append("High risk identified - consider specialist consultation before proceeding.")
        ...
        recommendations.append("Consider enhanced monitoring if procedure complexity increases.")
    else:  # low
    ...
    if risk_level == 'high':
        if block.blockId == 'CAD_CHF_001':
            recommendations.append("Consider cardiology consultation and optimization before surgery.")
            recommendations.append("Post-operative telemetry monitoring recommended.")
        elif block.blockId == 'DIABETES_001':
            recommendations.append("Consider endocrine consultation for glycemic optimization.")
            recommendations.append("Enhanced glucose monitoring perioperatively.")
        elif block.blockId == 'CKD_001':
            recommendations.append("Consider nephrology consultation if not already established.")
...
def _adjust_language_for_confidence(
    ...
    if confidence_tier == 'very_high':
        ...
        text = text.replace(f"Patient with {condition_name}", f"Patient HAS {condition_name}")
        text = text.replace(f"patient with {condition_name}", f"patient HAS {condition_name}")
        text = text.replace(f"{condition_name} present", f"{condition_name} CONFIRMED")
        # Remove qualifiers like "may", "consider", "if indicated"
        text = re.sub(r'\b(may|consider|if indicated|if appropriate)\b', '', text, flags=re.IGNORECASE)
        text = re.sub(r'\s+', ' ', text).strip()  # Clean up extra spaces
    
    elif confidence_tier == 'high':
        # Confident language: "Patient with [condition]"
        text = text.replace(f"Possible {condition_name}", f"Patient with {condition_name}")
        text = text.replace(f"possible {condition_name}", f"patient with {condition_name}")
        text = text.replace(f"{condition_name} cannot be ruled out", f"{condition_name} likely present")
        # Minor qualifiers: "consider", "if indicated"
        text = re.sub(r'\b(must|should|will)\b', lambda m: 'consider' if m.group().lower() == 'must' else m.group(), text, flags=re.IGNORECASE)
    
    elif confidence_tier == 'moderate':
        # Qualified language: "Possible CAD" or "History suggests CAD"
        text = text.replace(f"Patient HAS {condition_name}", f"Possible {condition_name}")
        text = text.replace(f"patient HAS {condition_name}", f"possible {condition_name}")
        text = text.replace(f"Patient with {condition_name}", f"History suggests {condition_name}")
        text = text.replace(f"patient with {condition_name}", f"history suggests {condition_name}")
        # Add qualifiers: "may benefit from", "consider"
        text = re.sub(r'\b(should|must|will)\b', lambda m: 'may benefit from' if m.group().lower() == 'should' else 'consider', text, flags=re.IGNORECASE)
    
    elif confidence_tier in ['low', 'very_low']:
        # Cautious language: "CAD cannot be ruled out"
        text = text.replace(f"Patient HAS {condition_name}", f"{condition_name} cannot be ruled out")
        text = text.replace(f"patient HAS {condition_name}", f"{condition_name} cannot be ruled out")
        text = text.replace(f"Patient with {condition_name}", f"{condition_name} cannot be ruled out")
        text = text.replace(f"patient with {condition_name}", f"{condition_name} cannot be ruled out")
        text = text.replace(f"Possible {condition_name}", f"{condition_name} cannot be ruled out")
        text = text.replace(f"possible {condition_name}", f"{condition_name} cannot be ruled out")
        # Strong qualifiers: "if confirmed", "pending further evaluation"
        text = re.sub(r'\b(should|must|will|recommend)\b', lambda m: 'if confirmed' if m.group().lower() == 'should' else 'pending further evaluation', text, flags=re.IGNORECASE)
    ...
    if confidence_tier == 'very_high':
        ...
        for rec in recommendations:
            ...
            if rec.lower().startswith(('require', 'must', 'should')):
                rec = rec.replace('Require', 'Consider', 1).replace('require', 'consider', 1)
                rec = rec.replace('Must', 'Should consider', 1).replace('must', 'should consider', 1)
                rec = rec.replace('Should', 'Consider', 1).replace('should', 'consider', 1)
            adjusted.append(rec)
        return adjusted
...
def generate_personalized_content(
    ...
    patient_data: PatientClinicalData,
    risk_level: Literal['low', 'intermediate', 'high'],
    confidence_tier: Literal['very_high', 'high', 'moderate', 'low', 'very_low'] = 'moderate',
    ...
    med_personalization = _get_medication_based_personalization(patient_data, block.blockId)
    ...
    device_personalization = _get_device_based_personalization(patient_data, block.blockId)
    ...
    if confidence_tier == 'very_high':
        ...
        adjusted_risk_recommendations = _adjust_recommendations_for_confidence(risk_recommendations, confidence_tier)
        adjusted_personalized_recommendations = _adjust_recommendations_for_confidence(personalized_recommendations, confidence_tier)
    elif confidence_tier == 'moderate':
    ...
    condition_name = block.conditionName.split('/')[0] if '/' in block.conditionName else block.conditionName
    adjusted_why = _adjust_language_for_confidence(block.contentSections.why, confidence_tier, condition_name)
    ...
    base_content_parts = [
        ...
        "Intra-operative considerations: " + "; ".join(block.contentSections.intraOpConsiderations),
        "Post-operative management: " + "; ".join(block.contentSections.postOpManagement),
    ]
    
    # Add risk-specific recommendations (confidence-adjusted)
    if adjusted_risk_recommendations:
        base_content_parts.append("Risk-specific recommendations: " + "; ".join(adjusted_risk_recommendations))
    
    # Add personalized recommendations (confidence-adjusted)
    if adjusted_personalized_recommendations:
        base_content_parts.append("Patient-specific considerations: " + "; ".join(adjusted_personalized_recommendations))
...
def _get_medication_based_personalization(
    ...
    acei_arb_arni = [
        med for med in patient_data.medications
        if any(term in med.class_.lower() for term in ['ace', 'arb', 'arni']) or
           any(term in med.name.lower() for term in ['ace inhibitor', 'arb', 'sacubitril', 'valsartan', 'entresto'])
    ]
    if acei_arb_arni:
    ...
    sglt2_inhibitors = [
        med for med in patient_data.medications
        if 'sglt2' in med.class_.lower() or
           any(term in med.name.lower() for term in ['dapagliflozin', 'empagliflozin', 'canagliflozin'])
    ]
    if sglt2_inhibitors:
        recommendations.append(
    ...
    anticoagulants = [
        med for med in patient_data.medications
        if any(term in med.class_.lower() for term in ['anticoagulant', 'antithrombotic']) or
           any(term in med.name.lower() for term in ['warfarin', 'apixaban', 'rivaroxaban', 'dabigatran', 'edoxaban', 'heparin', 'enoxaparin'])
    ]
    if anticoagulants:
...
def _get_lab_based_personalization(
    ...
    if egfr is not None and egfr < 30:
        recommendations.append(
            "Severe CKD - avoid nephrotoxins, contrast if possible"
        )
...
def _get_device_based_personalization(
    ...
    # Pacemaker/ICD
    if 'pacemaker' in [d.lower() for d in patient_data.devices] or 'ICD' in [d.upper() for d in patient_data.devices]:
        recommendations.append(
            "Device management: Ensure interrogation, magnet response known"
        )
    
    # CPAP
    if 'CPAP' in [d.upper() for d in patient_data.devices] or 'BIPAP' in [d.upper() for d in patient_data.devices]:
        recommendations.append(
            "Bring home CPAP/BiPAP device for postoperative use"
        )
...
def create_cad_chf_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        triggers=TriggerConditions(
            ...
            medicationTriggers=[
                MedicationTrigger(class_="nitrate", name_patterns=["nitroglycerin", "isosorbide"]),
                MedicationTrigger(class_="ranolazine", name_patterns=["ranolazine"]),
                MedicationTrigger(class_="loop_diuretic", name_patterns=["furosemide", "bumetanide", "torsemide"], min_dose=40.0, dose_unit="mg"),
                MedicationTrigger(class_="sglt2_inhibitor", name_patterns=["dapagliflozin", "empagliflozin", "canagliflozin"]),
                MedicationTrigger(class_="arni", name_patterns=["sacubitril", "valsartan", "entresto"]),
            ],
...
def create_osa_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        contentSections=ContentSections(
            ...
            preOpActions=["Ensure CPAP/BiPAP device available", "Continue CPAP/BiPAP perioperatively"],
            intraOpConsiderations=["Avoid sedatives that worsen OSA", "Consider CPAP post-extubation"],
            postOpManagement=["Resume CPAP/BiPAP immediately", "Monitor for respiratory depression"],
    ...
    return ComorbidityBlockDefinition(
        ...
        triggers=TriggerConditions(
            ...
            medicationTriggers=[
                ...
                MedicationTrigger(class_="sglt2_inhibitor", name_patterns=["dapagliflozin", "empagliflozin", "canagliflozin"]),
                MedicationTrigger(class_="glp1_agonist", name_patterns=["liraglutide", "semaglutide", "dulaglutide"]),
                MedicationTrigger(class_="oral_hypoglycemic", name_patterns=["metformin", "glipizide", "glyburide"]),
            ],
            labThresholds=[
        ...
        contentSections=ContentSections(
            ...
            preOpActions=["Optimize glucose control", "Consider endocrinology consultation if HbA1c > 8%"],
...
def create_ckd_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        contentSections=ContentSections(
            ...
            preOpActions=["Avoid nephrotoxic agents", "Optimize hydration", "Consider nephrology consultation if eGFR < 30"],
            intraOpConsiderations=["Avoid contrast if possible", "Monitor urine output", "Avoid hypotension"],
            postOpManagement=["Monitor creatinine", "Avoid nephrotoxins", "Ensure adequate hydration"],
...
def create_cirrhosis_liver_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        contentSections=ContentSections(
            ...
            whatToCheck=["INR", "Platelets", "Albumin", "Bilirubin", "Child-Pugh score"],
            preOpActions=["Consider hepatology consultation", "Optimize coagulation", "Avoid hepatotoxins"],
            intraOpConsiderations=["Avoid hepatically metabolized drugs", "Monitor for bleeding", "Avoid hypotension"],
            postOpManagement=["Monitor LFTs", "Avoid hepatotoxins", "Monitor for encephalopathy"],
...
def create_anemia_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        contentSections=ContentSections(
            ...
            preOpActions=["Consider iron supplementation", "Evaluate for bleeding source", "Consider hematology consultation if severe"],
            intraOpConsiderations=["Have blood products available", "Monitor for bleeding"],
            postOpManagement=["Monitor hemoglobin", "Continue iron supplementation if indicated"],
...
def create_delirium_prevention_block() -> ComorbidityBlockDefinition:
    ...
    return ComorbidityBlockDefinition(
        ...
        contentSections=ContentSections(
            ...
            whatToCheck=["Cognitive assessment", "Medication review", "Frailty assessment"],
            preOpActions=["Review medications for deliriogenic agents", "Consider geriatrics consultation", "Optimize sleep"],
            intraOpConsiderations=["Avoid benzodiazepines", "Minimize anticholinergics", "Use regional anesthesia when possible"],
            postOpManagement=["Early mobilization", "Reorient frequently", "Avoid restraints", "Optimize sleep"],
...
class ClinicalRecommendations:
    ...
    consultationRecommendations: List[ConsultationRecommendation]  # Consult types with urgency
...
def generate_recommendations(
    ...
    for block in triggered_blocks:
        ...
        if block_id == "CAD_CHF_001":
            cad_chf_recs = _generate_cad_chf_recommendations(patient_data, block_id, risk_level)
        ...
        # Diabetes specific recommendations
            ...
            diabetes_recs = _generate_diabetes_recommendations(patient_data, block_id, risk_level)
...
def _generate_cad_chf_recommendations(
    ...
    if patient_data.hpiRedFlags.chestPain or patient_data.hpiRedFlags.shortnessOfBreath:
        ...
        required.append("BNP")
        consultations.append(ConsultationRecommendation(
    ...
    if patient_data.recent_cardiac_event_months is not None and patient_data.recent_cardiac_event_months < 12:
        consultations.append(ConsultationRecommendation(
            specialty="cardiology",
            urgency="required",
    ...
    if risk_level == 'high':
        ...
        flags.append("Strongly consider delaying elective surgery until cardiac optimization")
...
class ActionItem:
    ...
    specificity: int = 0  # Higher = more specific (e.g., "Monitor K+" > "Monitor electrolytes")
    confidenceScore: float = 0.5  # Confidence score of the block that generated this action (0-1)
...
class ConflictResolution:
    ...
    resolution: Optional[str]  # 'resolved', 'requires_review', 'auto_merged', 'confidence_based'
...
def categorize_action(text: str) -> str:
    ...
    # Monitoring keywords
    if any(kw in text_lower for kw in ['monitor', 'check', 'assess', 'evaluate', 'observe', 'track']):
        return 'monitoring'
    
    # Medication keywords
    if any(kw in text_lower for kw in ['continue', 'hold', 'stop', 'start', 'adjust', 'medication', 'drug', 'dose']):
        return 'medication'
    
    # Consultation keywords
    if any(kw in text_lower for kw in ['consult', 'refer', 'specialist', 'cardiology', 'nephrology', 'endocrinology', 'hepatology']):
        return 'consultation'
    
    # Procedure keywords
    if any(kw in text_lower for kw in ['procedure', 'surgery', 'delay', 'postpone', 'modify', 'avoid contrast', 'bleed risk']):
        return 'procedure'
    
    # Patient instructions keywords
    if any(kw in text_lower for kw in ['bring', 'ensure', 'patient', 'home', 'device', 'cpap', 'bipap']):
...
def calculate_specificity(text: str) -> int:
    ...
    specific_labs = ['k+', 'na+', 'mg++', 'creatinine', 'bnp', 'troponin', 'hgb', 'inr', 'platelets']
    for lab in specific_labs:
        if lab in text_lower:
            score += 10
    
    # Specific medication names add points
    if any(char.isdigit() for char in text):  # Contains numbers (e.g., "40mg", "3-4 days")
        score += 5
...
def are_actions_duplicate(action1: str, action2: str) -> bool:
    ...
    stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'if', 'for', 'with', 'to', 'of', 'in', 'on', 'at'}
    ...
    if len(meaningful_words1) > 0 and len(meaningful_words2) > 0:
        ...
        similarity = len(common_meaningful) / max(len(meaningful_words1), len(meaningful_words2))
        if similarity > 0.7:
            return True
...
def are_actions_conflicting(action1: str, action2: str) -> bool:
    ...
    for pair in continue_hold_pairs:
        if pair[0] in a1_lower and pair[1] in a2_lower:
            ...
            med_keywords = ['beta-blocker', 'ace', 'arb', 'sglt2', 'anticoagulant', 'diuretic', 'insulin']
            for med in med_keywords:
                if med in a1_lower and med in a2_lower:
                    return True
    
    # Check for opposite procedure actions
    if ('delay' in a1_lower or 'postpone' in a1_lower) and ('proceed' in a2_lower or 'continue' in a2_lower):
        return True
...
def merge_blocks(
    ...
    for block in triggered_blocks:
        ...
        personalized_recommendations = personalized_content.get('personalizedRecommendations', [])
            ...
            if is_duplicate:
                ...
                if action.specificity > existing_item.specificity:
                    ...
                    merged_item = ActionItem(
                        ...
                        sourceBlocks=sorted(list(set(existing_item.sourceBlocks + action.sourceBlocks))),
                        specificity=action.specificity,
                        confidenceScore=max(existing_item.confidenceScore, action.confidenceScore),  # Use higher confidence
                    )
                ...
                if has_conflict:
                    ...
                    if resolution.resolution in ['resolved', 'confidence_based'] and not resolution.requiresClinicianDecision:
                        ...
                        resolved_item = ActionItem(
                            ...
                            category=action.category,
                            sourceBlocks=sorted(list(set(conflict_item.sourceBlocks + action.sourceBlocks))),
                            specificity=max(action.specificity, conflict_item.specificity),
                            confidenceScore=max(action.confidenceScore, conflict_item.confidenceScore),  # Use higher confidence
                        )
    ...
    requires_review = any(
        c.requiresClinicianDecision or c.resolution == 'requires_review' for c in conflicts
    )
